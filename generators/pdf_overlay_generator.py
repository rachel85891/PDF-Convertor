...
_FONT_CANDIDATES = (
    Path(__file__).resolve().parent.parent / "assets" / "fonts" / "NotoSansHebrew-Regular.ttf",
    Path(__file__).resolve().parent.parent / "assets" / "fonts" / "Rubik-Regular.ttf",
    Path(__file__).resolve().parent.parent / "assets" / "fonts" / "Assistant-Regular.ttf",
)
...
def _type_b_cell_values_from_headers(entry: AttendanceEntry, header_texts: list[str]) -> list[str]:
    ...
    for raw in header_texts:
        ...
        if "הערות" in h:
            ...
            out.append(entry.entry_time)
        elif "יציאה" in h:
            out.append(entry.exit_time)
        elif "שעות" in h or "סהכ" in h:
            out.append(entry.total_hours)
        else:
            out.append("")
    return out
